  ...
  methods: {
        ...
        <td
          ...
          <div class='cell'>
            <label class='check-box'>
              <input checked={ trData.checked } onClick={ this.check.bind(this, trData, index) } type='checkbox'/>
    ...
    renderCustomizeTd (column, trData, index) {
      return (
        <td
          class={ this.border ? `is-${column.align}` : `no-border is-${column.align}` }>
          <div class='cell'>{column.render({ row: trData, index })}
          </div>
        </td>
      )
    },
